...
import { PIX_KEY_TYPE_LABELS } from "@/lib/schemas/fields/paymentMethodSchema";
  ...
  async (
    ...
    return createResult(async () => {
      ...
      const data = {
        ...
        pix_tipo: pm?.pixKeyType
          ? (PIX_KEY_TYPE_LABELS[pm.pixKeyType] ?? pm.pixKeyType)
          : "",
        pix_chave: pm?.pixKey ?? "",
      ...
      for (const { appointment } of transport.appointments) {
        ...
        if (!unit) {
          unit = {
            ...
            unidade_saude_tipo: UNIT_TYPE_LABELS[u.unitType] ?? u.unitType,
        ...
        const companionKey =
          appointment.hasCompanion && appointment.companion
            ...
            : "none";
        const groupKey = `${appointment.patient.id}::${companionKey}`;

        const existing = unit._patientsById.get(groupKey);
        if (existing) {
          ...
          unit._patientsById.set(groupKey, paciente);
